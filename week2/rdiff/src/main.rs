...
fn read_file_lines(filename: &String) -> Result<Vec<String>, io::Error> {
    ...
    let mut res = vec![];
    // line type : Result<String, io::Error>
    ...
    for i in 0..m {
        for j in 0..n {
            if seq1.get(i).unwrap() == seq2.get(j).unwrap() {
                ...
                C.set(i + 1, j + 1, cmp::max(C.get(i, j + 1).unwrap(), C.get(i + 1, j).unwrap()));
            }
        }
    }
    C
...
fn main() {
    ...
    // Be sure to delete the #[allow(unused)] line above
    let lines1 = read_file_lines(filename1).expect("Failure: open ");
    let lines2 = read_file_lines(filename2).expect("Failure: open ");
