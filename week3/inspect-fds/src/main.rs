...
fn main() {
    ...
    let option_process = ps_utils::get_target(&target).expect(
        "Problems happen!");

    if let Some(process) = option_process {
        // println!("Found pid {}", process.pid);
        process.print();
        let child_processes = ps_utils::get_child_processes(process.pid).expect("Get child processes wrong!");
